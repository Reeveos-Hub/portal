    ...
    <div className="client-sidebar" style={{height:'100vh',position:'sticky',top:0,flexShrink:0,fontFamily:$.f}}>
      ...
      <div style={{width:PANEL,background:'#fff',borderRight:`1px solid ${$.bdr}`,display:'flex',flexDirection:'column'}}>
        ...
        <div style={{flex:1,padding:'10px 8px',overflowY:'auto'}}>
          ...
          {isShopActive&&<div style={{marginLeft:14,paddingTop:2,paddingBottom:4,display:'flex',flexDirection:'column',gap:1}}>
            {[{id:'shop-products',label:'Products'},{id:'shop-packages',label:'Packages & Courses'},{id:'shop-treatments',label:'Treatments'}].map(s=>{const sa=activeTab===s.id;return<button key={s.id} onClick={()=>onNav(s.id)} style={{width:'100%',textAlign:'left',padding:'6px 10px',borderRadius:8,border:'none',cursor:'pointer',fontSize:12,fontWeight:sa?600:500,color:sa?'#111':'#7A776F',background:sa?'rgba(17,17,17,0.06)':'transparent',fontFamily:$.f,transition:'all 150ms'}} onMouseEnter={e=>{if(!sa){e.currentTarget.style.background='rgba(232,228,221,0.5)';e.currentTarget.style.color='#2C2C2A'}}} onMouseLeave={e=>{if(!sa){e.currentTarget.style.background='transparent';e.currentTarget.style.color=sa?'#111':'#7A776F'}}}>{s.label}</button>})}
          </div>}
