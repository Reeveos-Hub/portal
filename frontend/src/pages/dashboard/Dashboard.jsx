...
/* ResponsiveGridLayout imported directly — width provided via useContainerWidth hook */
...
const Dashboard = () => {
  ...
  const { containerRef: gridContainerRef, width: containerWidth } = useContainerWidth({ initialWidth: 1200 })
  ...
  return (
    <div className="-m-6 lg:-m-8 flex flex-col h-[calc(100vh-4rem)] overflow-y-auto" style={{ fontFamily: theme.font.family }}>
      ...
      <div className="flex flex-1 min-h-0">
        ...
        {/* Grid */}
        <div ref={gridContainerRef} className="flex-1 overflow-y-auto p-4 lg:p-6">
          {editMode && (
            ...
            {visibleLayout.map(item => {
              ...
              return (
                <div key={item.i}
                  ...
                  {/* Drag handle — always visible. Subtle grip in normal mode, full controls in edit mode */}
                  <div className={`widget-drag-handle flex items-center justify-between px-3 shrink-0 ${
                    isLocked ? 'cursor-default' : 'cursor-grab active:cursor-grabbing'
                  } ${editMode ? 'py-1.5 border-b bg-gray-50/50' : 'py-1 bg-transparent'}`}
                    style={{ borderColor: editMode ? theme.border.light : 'transparent' }}>
                    <div className="flex items-center gap-2">
                      <Grip size={11} className={editMode ? 'text-gray-300' : 'text-gray-200'} />
                      {editMode && <span className="text-[10px] font-semibold text-gray-400">{def?.name}</span>}
                    </div>
                    {editMode && (
