  ...
  if (screen === 'payment') return (
    <div style={{ display: 'flex', gap: 40, maxWidth: 800, margin: '0 auto' }}>
      <div style={{ flex: 1 }}>
        ...
        <button onClick={() => setScreen('welcome')} style={{ width: '100%', marginTop: 24, padding: 14, background: T.forest, color: T.white, border: 'none', borderRadius: 8, fontFamily: F, fontSize: 14, fontWeight: 600, cursor: 'pointer' }}>Subscribe · {selectedPlan.price}/month</button>
  ...
  if (screen === 'welcome') return (
    <div style={{ textAlign: 'center', maxWidth: 600, margin: '0 auto', padding: '40px 0' }}>
      ...
      <div style={{ display: 'flex', gap: 20, marginTop: 40 }}>
        <button onClick={() => setScreen('templates')} style={{ flex: 1, padding: 24, border: `1px solid ${T.border}`, borderRadius: 12, background: T.white, cursor: 'pointer', textAlign: 'left' }} onMouseOver={e => e.currentTarget.style.borderColor = T.gold} onMouseOut={e => e.currentTarget.style.borderColor = T.border}>
        ...
        <button onClick={() => setScreen('domain')} style={{ flex: 1, padding: 24, border: `1px solid ${T.border}`, borderRadius: 12, background: T.white, cursor: 'pointer', textAlign: 'left' }} onMouseOver={e => e.currentTarget.style.borderColor = T.gold} onMouseOut={e => e.currentTarget.style.borderColor = T.border}>
    ...
    return (
      <div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 16 }}>{list.map(t => (
          <div key={t.id} onClick={() => { setSelectedTemplate(t); setScreen('preview') }} style={{ border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden', background: T.white, cursor: 'pointer', transition: 'all 0.15s' }} onMouseOver={e => { e.currentTarget.style.borderColor = T.gold; e.currentTarget.style.transform = 'translateY(-2px)' }} onMouseOut={e => { e.currentTarget.style.borderColor = T.border; e.currentTarget.style.transform = 'none' }}>
    ...
    <div>
      ...
      {tab === 'home' && <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(5, 1fr)', gap: 12 }}>
          {pages.map(p => <div key={p.slug||p._id} onClick={() => navigate(`/dashboard/website/edit/${p.slug}`)} style={{ background: T.white, border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden', cursor: 'pointer' }} onMouseOver={e => e.currentTarget.style.borderColor = T.gold} onMouseOut={e => e.currentTarget.style.borderColor = T.border}><div style={{ height: 80, background: T.bg }} /><div style={{ padding: 8, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}><span style={{ fontFamily: F, fontSize: 11, fontWeight: 600, color: T.text }}>{p.title||p.slug}</span><span style={{ fontFamily: F, fontSize: 8, color: p.status==='published'?T.green:T.amber, fontWeight: 600, padding: '1px 4px', background: p.status==='published'?'#DCFCE7':'#FEF3C7', borderRadius: 4 }}>{p.status==='published'?'LIVE':'DRAFT'}</span></div></div>)}
          <div onClick={() => setTab('pages')} style={{ background: T.white, border: `2px dashed ${T.border}`, borderRadius: 12, display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', minHeight: 110 }} onMouseOver={e => e.currentTarget.style.borderColor = T.gold} onMouseOut={e => e.currentTarget.style.borderColor = T.border}><Ic d={ic.plus} size={20} /></div>
        </div>
      </>}
      ...
      {tab === 'pages' && <>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 14 }}>
          ...
          <div onClick={() => { const t = prompt('Page title:'); if(t) createPage(t, t.toLowerCase().replace(/\s+/g,'-').replace(/[^a-z0-9-]/g,'')) }} style={{ background: T.white, border: `2px dashed ${T.border}`, borderRadius: 12, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', minHeight: 200 }} onMouseOver={e => e.currentTarget.style.borderColor = T.gold} onMouseOut={e => e.currentTarget.style.borderColor = T.border}><Ic d={ic.plus} size={22} /><p style={{ fontFamily: F, fontSize: 11, color: T.muted, marginTop: 6 }}>New Page</p></div>
        </div>
      </>}
