...
This is a local services business (salon/clinic/spa), NOT a restaurant. Use "appointments" not "bookings", "clients" not "guests", "therapist/staff" not "server". Answer questions about today's schedule, availability, services, etc. Be specific with numbers.`)
  ...
  return (
    <div style={{ fontFamily: '"Figtree", system-ui, sans-serif' }}>
      ...
      {/* Chat Window — portal-rendered for inpage/fullpage, fixed for popup */}
      {isOpen && (() => {
        const chatPanel = (
          <div
            className={(!chatMode || chatMode === 'popup') ? "chat-window-enter" : ""}
            style={(!chatMode || chatMode === 'popup') ? {
              ...
              /* inpage + fullpage: fill the portal container */
          ...
          >
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div
                  style={{
                    ...
                    background: "#111111", border: "2px solid #C9A84C",
                    display: "flex", alignItems: "center", justifyContent: "center",
                    color: "#C9A84C", fontFamily: '"Figtree", sans-serif', fontWeight: 800, fontSize: 17,
                  }}
                >R</div>
                <div>
                  <div style={{ color: "white", fontFamily: '"Figtree", sans-serif', fontWeight: 800, fontSize: 15 }}>
                    ReeveOS Assistant
                  </div>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: 2 }}>
                ...
                {/* Expand: inpage → fullpage, Shrink: inpage → popup */}
                {chatMode === 'inpage' && (
                  <>
                    ...
                    <button onClick={() => setChatMode('popup')} title="Popup" style={{ background: "none", border: "none", color: "#888", cursor: "pointer", padding: 6, borderRadius: 6 }}>
        ...
        // Portal routing: inpage/fullpage render into DashboardLayout containers
        if (chatMode === 'inpage') {
          const target = document.getElementById('assistant-inpage-portal')
          return target ? createPortal(chatPanel, target) : chatPanel
        }
        if (chatMode === 'fullpage') {
          const target = document.getElementById('assistant-fullpage-portal')
          return target ? createPortal(chatPanel, target) : chatPanel
        }
        return chatPanel
      })()}
